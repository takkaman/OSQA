...
from forum.utils.checksnpsid import isidvalid
import logging
...
class TagNamesField(forms.CharField):
    def __init__(self, user=None, *args, **kwargs):
        ...
        self.required = True
        self.widget = forms.TextInput(attrs={'size' : '100%', 'autocomplete' : 'off'})
        self.max_length = 255
        self.label  = _('Tags')
...
class UserSearchField(forms.CharField):
    def __init__(self, *args, **kwargs):
        super(UserSearchField, self).__init__(*args, **kwargs)

        self.required = True
        self.max_length = 255
        self.widget = forms.TextInput(attrs={'size' : 70, 'autocomplete' : 'off', 'maxlength' : self.max_length})
        self.label  = _('search')
        self.help_text = _('search ..')
        self.initial = ''
...
class InviteNamesField(forms.CharField):
    def __init__(self, user=None, *args, **kwargs):
        super(InviteNamesField, self).__init__(*args, **kwargs)

        self.required = False
        self.widget = forms.TextInput(attrs={'size' : 100, 'autocomplete' : 'off'})
        self.max_length = 255
        self.label  = _('invites')
        #self.help_text = _('please use space to separate invites (this enables autocomplete feature)')
        self.help_text = _('Invites are SNPS ids, with no spaces within. At least %(min)s and up to %(max)s invites can be used.') % {
            'min': settings.FORM_MIN_NUMBER_OF_INVITES, 'max': settings.FORM_MAX_NUMBER_OF_INVITES    
        }
        self.initial = ''
        self.user = user

    def clean(self, value):
        super(InviteNamesField, self).clean(value)

        value = super(InviteNamesField, self).clean(value)
        data = value.strip().lower()

        split_re = re.compile(r'[ ,]+')
        list = {}
        err_list = []
        for invite in split_re.split(data):
            list[invite] = invite
            if invite != '' and not isidvalid(invite):
                err_list.append(invite) 

        if len(list) > settings.FORM_MAX_NUMBER_OF_INVITES or len(list) < settings.FORM_MIN_NUMBER_OF_INVITES:
            raise forms.ValidationError(_('please use between %(min)s and %(max)s invites') % { 'min': settings.FORM_MIN_NUMBER_OF_INVITES, 'max': settings.FORM_MAX_NUMBER_OF_INVITES})

        if len(err_list) > 0:
            raise forms.ValidationError(_('invalid user: \'%s\'' %(' '.join(err_list)) ))
        
        list_temp = []
        invitename_re = re.compile(r'^[\w+#\.-]+$', re.UNICODE)
        empty_re = re.compile(r'^\s*$', re.UNICODE)
        for key,invite in list.items():
            if len(invite) > settings.FORM_MAX_LENGTH_OF_INVITE or len(invite) < settings.FORM_MIN_LENGTH_OF_INVITE:
                raise forms.ValidationError(_('please use between %(min)s and %(max)s characters in your userid') % { 'min': settings.FORM_MIN_LENGTH_OF_INVITE, 'max': settings.FORM_MAX_LENGTH_OF_INVITE})
            if empty_re.match(invite):
                continue
            if not invitename_re.match(invite):
                raise forms.ValidationError(_('please use following characters in userids: letters , numbers, and characters \'.#-_\''))
            # only keep one same invite
            if invite not in list_temp and len(invite.strip()) > 0:
                list_temp.append(invite)
        ...
        if settings.LIMIT_TAG_CREATION and not self.user.can_create_tags():
            ...
            if len(existent) < len(list_temp):
                unexistent = [n for n in list_temp if not n in existent]
                raise forms.ValidationError(_("You don't have enough reputation to create new invites. The following invites do not exist yet: %s") %
                        ', '.join(unexistent))
...
class AskForm(forms.Form):
    ...
    def __init__(self, data=None, user=None, space_id=1, *args, **kwargs):
        super(AskForm, self).__init__(data, *args, **kwargs)

        self.fields['tags']   = TagNamesField(user)
        self.fields['invites'] = InviteNamesField(user)
        self.fields['usersearch'] = UserSearchField()
        self.fields['public'] = forms.ChoiceField(widget=forms.RadioSelect, choices=((1,_('public')), (0, _('private'))), initial=1)
        self.fields['space'] = forms.ChoiceField(widget=forms.Select(attrs={'style' : 'width:100%'}))
        self.fields['space'].choices = [(s.id, s.space_name) for s in Space.objects.all().order_by('id')]
...
class AnswerForm(forms.Form):
    ...
    def __init__(self, data=None, user=None, *args, **kwargs):
        ...
        self.fields['invites'] = InviteNamesField(user)
        self.fields['usersearch'] = UserSearchField()
        #self.fields['public'] = forms.ChoiceField(widget=forms.RadioSelect, choices=((True,_('public')), (False, _('private'))), initial=True)
...
class EditQuestionForm(forms.Form):
    ...
    def __init__(self, question, user, revision=None, *args, **kwargs):
        ...
        # else:
        #     initial_public = 0
        self.fields['space'] = forms.ChoiceField(widget=forms.Select(attrs={'style' : 'width:520px'}))
        self.fields['space'].choices = [(s.id, s.space_name) for s in Space.objects.all()]
        self.fields['space'].initial = question.space.id
        initial_public = int(revision.public)
        self.fields['public'] = forms.ChoiceField(
                widget=forms.RadioSelect,
                choices=((1, _('public')), (0,_('private'))),
                initial=initial_public)
        self.fields['invites'].initial = ' '.join([user.username for user in question.whitelist.all()])
...
class EditUserForm(forms.Form):
    ...
    website = forms.CharField(label=_('Website'), required=False, max_length=255, widget=forms.TextInput(attrs={'size' : 35}))
    ...
    skills = forms.CharField(label=u'Skills', required=False, max_length=255, widget=forms.TextInput(attrs={'size' : 35}))

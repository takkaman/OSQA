...
def question_posted(action, new):
    ...
    if getattr(question, 'invites', None):
        idlist = []
        for userid in question.invites.split():
            user = User(username=userid, id=999L, email=(userid+ u'@synopsys.com'), real_name=userid, email_isvalid=True)
            idlist.append(user)
        send_template_email(idlist, "notifications/newquestion.html", {'question': question})
...
def answer_posted(action, new):
    ...
    if question.public:
        ...
        if getattr(answer, 'invites', None):
            idlist = []
            for userid in answer.invites.split():
                user = User(username=userid, id=999L, email=(userid+ u'@synopsys.com'), real_name=userid, email_isvalid=True)
                idlist.append(user)
            send_template_email(idlist, "notifications/newanswer.html", {'answer': answer})
        create_subscription_if_not_exists(question, answer.author)
    else:
        idlist = []
        idlist.append(question.author)
        for user in question.whitelist.all():
            idlist.append(user)
        send_template_email(idlist, "notifications/newanswer.html", {'answer': answer})
...
def comment_posted(action, new):
    ...
    if question.public:
        ...
        idlist = []
        idlist.append(question.author)
        print "Leo debug =>", question.whitelist.all()
        for user in question.whitelist.all():
            idlist.append(user)
        send_template_email(idlist, "notifications/newcomment.html", {'comment': comment})   
...
def answer_accepted(action, new):
    ...
    if question.public:
        ...
        for user in question.whitelist.all():
            idlist.append(user)
        send_template_email(idlist, "notifications/answeraccepted.html", {'answer': action.node})   

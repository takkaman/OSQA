...
from forum.models import Comment, Question, Answer, NodeRevision, User, Space
...
class NodeEditAction(ActionProxy):
    def create_revision_data(self, initial=False, **data):
        if int(data.get('space', u'0')) == 0:
            space = None
        else:
            space = Space.objects.get(id=int(data.get('space', u'0')))

        revision_data = dict(summary=data.get('summary', (initial and _('Initial revision') or '')), body=data['text'], public=(data.get('public', u'0') == u'1'), space=space)
            ...
            for inviter_name in question.invites.split():
                try:
                    ...
                    if isidvalid(inviter_name):
                        inviter = User(username=inviter_name, email=(inviter_name+ u'@synopsys.com'), real_name=inviter_name, email_isvalid=True)
                        inviter.save()
                    else:
                        continue
...
class CommentAction(ActionProxy):
    ...
    def process_data(self, text='', parent=None):
        ...
        match = re.match(r'<p>Reply (\w+):', text)
        if match != None:
            userid = match.group(1)
            try:
                user = User.objects.get(username=userid)
                send_template_email([user], 'notifications/comment.html', {'comment': comment})
            except User.DoesNotExist:
                pass
...
class ReviseAction(NodeEditAction):
    ...
    def process_data(self, **data):
        ...
        if 'invites' in data:
            current_invites = [user.username for user in self.node.whitelist.all()]
            invites_to_add = set(data['invites'].strip().split()).difference(set(current_invites)) 
            idlist = []
            for userid in invites_to_add:
                try:
                    inviter = User.objects.get(username=userid)
                except User.DoesNotExist:
                    if isidvalid(userid):
                        ...
                        continue
                self.node.whitelist.add(inviter)
                inviter.save()
                idlist.append(inviter)

            send_template_email(idlist, 'notifications/newquestion.html', {'question': self.node})

            invites_to_remove = set(current_invites).difference(set(data['invites'].strip().split())) 
            for userid in invites_to_remove:
                try:
                    inviter = User.objects.get(username=userid)
                except User.DoesNotExist:
                    if isidvalid(userid):
                        ...
                        continue
                self.node.whitelist.remove(inviter)
                inviter.save()

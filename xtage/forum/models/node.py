...
from forum.models.space import Space
...
class NodeContent(models.Model):
    ...
    whitelist  = models.ManyToManyField(User, related_name='access_%(class)ss')
    space      = models.ForeignKey(Space, related_name='access_%(class)ss', null=True)
...
class Node(BaseModel, NodeContent):
    ...
    def activate_revision(self, user, revision):
        ...
        self.public = revision.public
        self.space = revision.space
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            ...
            self.active_revision = self._create_revision(self.author, 1, title=self.title, tagnames=self.tagnames, body=self.body, public=self.public, space=self.space)

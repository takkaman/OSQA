...
var response_commands = {
    ...
    update_skill_adore: function(type, id, count) {
        var $skill_adore = $('#skill-adore-' + id);
        var $skill_adore_count = $('#skill-adore-count-' + id);
        if (type == 'on') {
            $skill_adore.addClass('on');
        } else {
            $skill_adore.removeClass('on');
        }
        $skill_adore_count.text(count);
    },
    ...
    $('div.comment-form-container').each(function() {
        ...
        if ($form.length) {
            ...
            function process_form_changes() {
                ...
                $body = $body.replace(/<[^>]+>/g, ""); 
                var length = $body.replace(/[ ]{2,}/g," ").length;
            ...
            if($comments_container.find('.comment-reply').length) {
                $comments_container.find('.comment-reply').click(function(){
                    cleanup_form();
                    show_comment_form();
                    var editor_id = $container.find('textarea').attr('id')
                    ...
                    tinyMCE.get(editor_id).setContent('Reply ' + user_name + ': ');
                });
            }

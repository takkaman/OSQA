...
def upload(request):#ajax upload file to a question or answer
    ...
    try:
        ...
        result = {}  
        result['path']  = os.path.join(str(settings.UPFILES_ALIAS), new_file_name)
        result['error'] = False
    except UploadPermissionNotAuthorized:
        #result = xml_template % ('', _('uploading images is limited to users with >60 reputation points'), '')
        #result['error'] = True
        result['error'] = 'uploading images is limited to users with >60 reputation points'
        ...
        #result['error'] = True
        result['error'] = "maximum upload file size is %d m" % settings.ALLOW_MAX_FILE_SIZE
    except Exception, e:
        #result = xml_template % ('', _('Error uploading file. Please contact the site administrator. Thank you. %s' % e), '')
        #result['error'] = True
        result['error'] = 'Error uploading file. Please contact the site administrator. Thank you.' 
    return HttpResponse(json.dumps(result), content_type='text/html')

def ask(request, space_id=1):
    form = None
    ...
    elif request.GET.get('qid',None) is not None:
        qid = int(request.GET.get('qid',None))
        model = get_object_or_404(Question, id=qid)
        form = EditQuestionForm(model, request.user)
        form.fields['title'].initial = ''
        form.fields['invites'] = InviteNamesField(request.user)
        form.fields['invites'].initial = ' '.join([u.username for u in model.whitelist.all()])
            
    if not form:
        form = AskForm(user=request.user, space_id=space_id)

    return render_to_response('ask.html', {
        'form' : form,
        'tab' : 'ask',
        'tags': Tag.active.order_by('-id')[:settings.RECENT_TAGS_SIZE],
        'd_tags': settings.PREDEFINED_TAGS,
        'space_id': space_id
...
def answer(request, id):
    ...
    if request.POST:
        ...
        if request.user.is_authenticated() and request.user.email_valid_and_can_answer():
            ...
            if question.public:
                answer_public = u'1'
            answer_action = AnswerAction(user=request.user, ip=request.META['REMOTE_ADDR']).save(dict(question=question, public=answer_public, **form.cleaned_data))

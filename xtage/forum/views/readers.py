...
class WikiQuestionsSort(pagination.SortBase):
    def apply(self, questions):
        return questions.extra(select={
            'wiki_count': '''
            SELECT COUNT(1)
                FROM forum_node fn1
                WHERE (fn1.abs_parent_id = forum_node.id
                    OR  fn1.id = forum_node.id)
                    AND (fn1.state_string LIKE '%%(wiki)%%')
                    ''',
            }
        ).order_by('-wiki_count', '-last_activity_at')
    ...
    def apply(self, questions):
        return questions.extra(select={'new_child_count': '''
            ...
            select_params=[ (datetime.datetime.now() - datetime.timedelta(days=2))
                .strftime('%Y-%m-%d')]
        ).order_by('-wiki_count', '-new_child_count', 'last_activity_at')
        ...
        return questions.extra(select={'answer_count': '''
            ...
            SELECT COUNT(1)
                ...
                WHERE (fn1.abs_parent_id = forum_node.id
                    ...
                    AND (fn1.state_string LIKE '%%(wiki)%%')
                    ''',
            }).order_by('-wiki_count', 'answer_count', 'last_activity_at')
...
class QuestionListPaginatorContext(pagination.PaginatorContext):
    def __init__(self, id='QUESTIONS_LIST', prefix='', pagesizes=(15, 30, 50), default_pagesize=30):
        super (QuestionListPaginatorContext, self).__init__(id, sort_methods=(
            (_('wiki'), WikiQuestionsSort(_('wiki'), _("list <strong>Wiki</strong> questions on top</strong>"))),
            (_('active'), pagination.SimpleWikiSort(_('active'), '-last_activity_at', _("Most <strong>recently updated</strong> questions"))),
            (_('newest'), pagination.SimpleWikiSort(_('newest'), '-added_at', _("most <strong>recently asked</strong> questions"))),
            (_('hottest'), HottestQuestionsSort(_('hottest'), _("most <strong>active</strong> questions in the last 24 hours"))),
            (_('mostvoted'), pagination.SimpleWikiSort(_('most voted'), '-score', _("most <strong>voted</strong> questions"))),
...
def feed(request):
    return RssQuestionFeed(
                request,
                Question.objects.filter(public=True).filter_state(deleted=False).order_by('-last_activity_at'),
                settings.APP_TITLE + _(' - ')+ _('latest questions'),
                settings.APP_DESCRIPTION)(request)

def get_questions(user):
    public_questions = Question.objects.filter(public=True)
    if user.is_authenticated():
        ...
        group_qustions = involved_group_questions | created_group_questions
        questions = (group_qustions | public_questions).distinct()
    else:
...
def unanswered(request):
    return question_list(request,
                         Question.objects.filter(public=True).exclude(id__in=Question.objects.filter(children__marked=True).distinct()).exclude(marked=True),
...
def space(request, id, slug=''):
    try:
        space = Space.objects.get(id=id)
    except:
        raise Http404 
    return question_list(request,
                         get_questions(request.user).filter(space__id=id),
                         _('questions'),
                         None,
                         mark_safe(_(u' %(space)s') % {'space': space.space_name}),
                         space_id=space.id)
...
def tag(request, tag):
    ...
    # questions = Question.objects.filter(public=True).filter(tags__id=tag.id)
    questions = get_questions(request.user).filter(tags__id=tag.id)
...
def search(request):
    if request.method == "GET" and "q" in request.GET:
        ...
        if search_type == 'tag':
            ...
            return HttpResponseRedirect(reverse('skills') + '?q=%s' % urlquote(keywords.strip()))
        else:
...
def question_search(request, keywords):
    ...
    # Now question's title and content can be searched
    initial  = Question.objects.filter(public=True).filter(Q(title__icontains=keywords) | Q(body__icontains=keywords))
    can_rank = False
...
def question(request, id, slug='', answer=None):
    ...
    if not question.public and (request.user != question.author and not request.user in question.whitelist.iterator() and not request.user.is_superuser):
    ...
    # If a public question has some private answer, the answer is changed to
    # public here. 
    # The better way is to change the state when the question is made to public.
    if question.public is True:
        for answer in answers:
            if answer.public is False:
                answer.public = True
                answer.save()

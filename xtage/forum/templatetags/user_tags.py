...
@register.inclusion_tag('users/skill_adore.html')
def skill_adore(skillownership, viewer, user):
    print "target user_skill is ", skillownership, skillownership.id

    adorer = skillownership.adores.all().filter(id=user.id)
    if adorer.count():
        print "Found user adore record", user.id, user.username
        favorited = True
    else:     
        print "Not found user record", user.id, user.username
        favorited = False

    return {'favorited': favorited, 'user_skill': skillownership}
